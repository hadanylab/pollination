function [ BD,AB,BC,CD,BD_border_CM,AB_border_CM,BC_border_CM,CD_border_CM ] = CM_find_borders_diff_g(param1,param2,param3)
...
%eq = g1*(1/F1-1/Fmax)==g2*(1/F2-1/Fmax);
%F1_exp = simplify(solve(eq,F1));
%gives this:
F1_exp = (F2*Fmax*g1)/(F2*g1 - F2*g2 + Fmax*g2);

%eq4 = Vc == Vr
eq4 = QTotal*V3/D == g2*(1/F2-1/Fmax);
eq4 = subs(eq4,[p1],[1-p2]);
F1_solved = solve(eq4,F1,'ReturnConditions',true,'IgnoreAnalyticConstraints',true); %doesn't solve under the real-positive assumptions on F1-F2
F1_solved = simplify(F1_solved.F1,'Steps',50,'IgnoreAnalyticConstraints',true);

%eq5 = Fi_1==Fi_3
eq5 = F1*V1 == (p1*F1+p2*F2)*V3;
%eq5 = 1/V1==1/V3;
eq5 = subs(eq5,[p1],[1-p2]);
...
if (isempty(F2_other.F2))
    ...
    F2_other = F2_other(F2_other~=0);
    
    eq = subs(F1==F1_exp,[F2],[F2_other]);
    F1_one_side = simplify(solve(eq,F1));
    F1_one_side = F1_one_side(F1_one_side~=0);
    
    eq = subs(F1==F1_solved,[F2],[F2_other]);
    F1_second_side = simplify(solve(eq,F1));
    F1_second_side = F1_second_side(F1_second_side~=0);
    
    %JUST PICKED ONE, I DON'T KNOW IF IT MATTERS
    F1_second_side = F1_second_side(2); 
    
    eq9 = F1_one_side == F1_second_side;
end
AB_border_CM = eq9;
AB = solve(eq9,p2,'ReturnConditions',false);
...
BD = solve(eq2,p2,'ReturnConditions',false,'IgnoreAnalyticConstraints',false);
...
%(QTotal*V1)/(D*p1)==g1*(1/F1-1/Fmax)
%eq2 = F1_solved == Fmax/(1+(QTotal*Fmax*V1_tag)/(D*g1*(1-p2)));
eq2 = F1_solved == 1/(1/Fmax + (QTotal*V1_tag)/(D*g1*p1));
...
BC_border_CM = eq3;
BC = solve(eq3,p2,'ReturnConditions',false,'IgnoreAnalyticConstraints',false);
...
CD_border_CM = eq3;
CD = solve(eq3,p2,'ReturnConditions',false,'IgnoreAnalyticConstraints',false);
